...
pub fn generate_profile(
    ...
    for lib in libs {
        if let Some(file) = &lib.downloads.artifact {
            ...
            if let Some(rules) = &lib.rules {
                for rule in rules {
                    let optional_name = [lib.name.as_str(), rule.action.as_str(), rule.os.as_ref().map(|os| os.name.as_str()).unwrap_or("any")].join("_");
                    if optionals.iter().find(|o| o.name.as_ref().map(|n| n.eq(&optional_name)).unwrap_or(false)).is_some() {
                        continue;
                    }
                    ...
                    let optional = Optional {
                        ...
                        enabled: true,
                        visible: false,
                        description: Some("Autogenerated optional from profile-generator".to_string()),
    ...
    match game_type {
        Fabric(mut fabric_manifest) => {
            ...
            lib_path.pop();
            let path = libraries_folder.join(lib_path);
            download_file(&mappings_url, path)?;
